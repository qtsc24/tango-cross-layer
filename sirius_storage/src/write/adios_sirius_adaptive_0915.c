...
void BitplaneTransfer(double * bitblock, double ** tbitblock, int n, int blockid)
{
    double * bitblock_new = (double *)malloc (8 * 64);
    memset(bitblock_new, 0, sizeof(double) * 64);
    unsigned long long tmp_p = 0;
    for (int i = 0; i < 64; i++){
        unsigned long long p = * (unsigned long long *) &bitblock[i];
        /*
        if (blockid == 0 && i == 1 && n ==2){
            printf("*********************\n");
            PrintDouble(bitblock[i], 0);
            PrintDoubleBinary(p);
            printf("*********************\n");
        }
        */
        ...
        for (int j = 63; j >= 0; j--)
        {
            ...
            //PrintDoubleBinary(p);
            tmp_p = p >> j;
            //PrintDoubleBinary(tmp_p);
            tmp_p = tmp_p & 0b1;
            //PrintDoubleBinary(tmp_p);
            //printf("Before:");
            unsigned long long p_new = * (unsigned long long *) &bitblock_new[63 - j];
            //PrintDoubleBinary(p_new);
            p_new = p_new<<1;
            //printf("Middle:");
            //PrintDoubleBinary(p_new);
            p_new = p_new | tmp_p;
            //printf("After:");
            //PrintDoubleBinary(p_new);
            memcpy(&bitblock_new[63 - j], &p_new, 8);
            //PrintDouble(bitblock_new[63 - j], 0);
            //printf("p_new[%d] = %llu\n", j, p_new);
            //printf("###########################");
        }
    }
    ...
    if (blockid == 0){
        printf("########################\n");
        printf("Bitplane #: %d\n", n);
        printf("bitblock_new[1] = %f\n", bitblock_new[1]);
        PrintDouble(bitblock_new[1], 0);
        printf("########################\n");
    }
    */
    /*
    for (int i = 0; i < 64; i++){
        unsigned long long p_new = * (unsigned long long *) &bitblock_new[i];
        printf("After:");
        PrintDoubleBinary(p_new);
    }
    */
    ...
    printf("################\n");
    for (int j= 0; j < 64; j++)
        PrintDouble(bitblock_new[j], j);
    printf("################\n");
    */
    * tbitblock = bitblock_new;

}
void DoubleToBitplaneBlock(double * src, int n, double ** delta_transformed, int * delta_transformed_n){
    * delta_transformed_n = (n/64+1)*64;
    double * delta_extend = (double *)malloc (*delta_transformed_n * 8);
    memset(delta_extend, 0, *delta_transformed_n * sizeof(double));
    double * bitblock = (double *)malloc (8 * 64);
    double * bitblock_new;
    for (int i = 0; i < n; i += 64){
        if ((i + 64) >= n){
            memset(bitblock, 0, sizeof(double) * 64);
            memcpy(bitblock, src + i, 8 * (n - i));
        }
        else
            memcpy(bitblock, src + i, 8*64);
        BitplaneTransfer(bitblock, &bitblock_new, 52, i);

        memcpy(delta_extend + i, bitblock_new, 8*64);
    }
    * delta_transformed = delta_extend;
}

void BitplaneToDouble(double *src, int src_n, int n, int n_bitplane, double ** des){
    double * bitblock = (double *)malloc (8 * 64);
    double * bitblock_new = (double *)malloc (8 * 64);
    memset(bitblock_new, 0, sizeof(double) * 64);
    double * src_temp = (double *)malloc (8 * src_n);
    double * delta_new = (double *)malloc (8 * n);
    assert(src_temp && bitblock && bitblock_new && delta_new);
    memcpy (src_temp, src, src_n * 8);
    for (int i = 0; i < src_n; i+=64){
        memcpy(bitblock, src + i, 8*64);
        BitplaneTransfer(bitblock, &bitblock_new, n_bitplane, i);
        if (i + 64 >= src_n )
            memcpy(delta_new + i, bitblock_new, 8*(src_n - i));
        else
            memcpy(delta_new + i, bitblock_new, 8*64);
    }
    * des = delta_new; 
}
...
void encode_block(uint64_t const * data, size_t n, uint8_t num_bitplanes, uint64_t* streams, int bitplane_size, int blockid){
    for(int k=num_bitplanes - 1; k>=0; k--){
        ...
        for (int i=0; i<n; i++){
            bitplane_value += (uint64_t)((data[i] >> k) & 1u) << i;
        }
        streams[bitplane_index * bitplane_size + blockid] = bitplane_value;
        if (blockid == 0){
            printf("bitplane_value = %lu\n",bitplane_value);
            printf("%lu\n", streams[bitplane_index * bitplane_size + blockid]);
        }
        //*(streams[bitplane_index] ++) = bitplane_value;
        //printf("Dif = %d\n", (streams[bitplane_index]) - (streams[bitplane_index]));
        //streams[bitplane_index] + 1 = bitplane_value;
    }
}
...
{   
    ...
    //int bitplane_size = n / 8 + sizeof(uint64_t);
    //uint64_t * streams[num_bitplanes];
    //for (int i = 0; i < num_bitplanes; i ++){
    //    streams[i] = (uint64_t *) malloc (bitplane_size * sizeof(uint64_t)); 
        //printf("streams[%d] = %p\n", i, streams[i]);
        //if (i != 0)   printf("streams[%d] length = %d\n", i - 1, streams[i] - streams[i-1]);
    //}
    
    //uint64_t * streams = (uint64_t *)malloc(bitplane_size * num_bitplanes);    
    ...
    for (int i = 0; i < block_size; i++){
        printf("streams_pos[%d] = %lu\n", i, streams[i*bitplane_size+1]);
    }
    return streams;
}
...
void decode_block(uint64_t const * streams, size_t n, uint8_t num_bitplanes, uint64_t * data, int bitplane_size, int blockid) {
    for(int k=num_bitplanes - 1; k>=0; k--){
        ...
        //uint64_t bitplane_value = *(streams++);
        for (int i=0; i<n; i++){
...
double * NegaDecode(uint64_t * streams, int n, int exp, int num_bitplanes) {
    ...
    if(ending_bitplane % 2 == 0){
        for(int i=0; i<n - block_size; i+=block_size){
            ...
            for(int j=0; j<block_size; j++){
                *(data++) = ldexp((double) negabinary2binary(int_data_buffer[j]), - ending_bitplane + exp);
            }
        }
        {
            int rest_size = n % block_size;
...
//#ifdef CEPH
            ...
            */
            ...
            printf("PSNR = %f\n",psnr);
            double target_nrmse[5] = {0.1, 0.01, 0.001, 0.0001, 0.00001};
            ...
            */
            ...
            double absmax = 0.0;
            for (int i = 0; i < nelems; i++){
                if (fabs(delta[i]) > absmax)    absmax = fabs(delta[i]);
            }
            
            int level_exp = 0;
            frexp(absmax, &level_exp);   
            printf("level_exp = %d\n", level_exp);
            int num_bitplanes = 64;     
            double start_encode = MPI_Wtime ();   
            uint64_t* encoded_bitplanes, encoded_bitplanes_r, encoded_bitplanes_z; 
            encoded_bitplanes = NegaEncode(delta, nelems, level_exp, num_bitplanes);
            //encoded_bitplanes_r = NegaEncode(delta_r, nelems, level_exp, num_bitplanes);
            //encoded_bitplanes_z = NegaEncode(delta_z, nelems, level_exp, num_bitplanes);
            double end_encode = MPI_Wtime ();  
            //for(int i = 1000; i < 2000; i ++)
            //    printf("streams[%d] = %d\n", i, encoded_bitplanes[i]);
         
            double * decoded_bitplanes, decoded_bitplanes_r, decoded_bitplanes_z;
            decoded_bitplanes = NegaDecode(encoded_bitplanes, nelems, level_exp, 16);
            //decoded_bitplanes_r = NegaDecode(encoded_bitplanes_r, nelems, level_exp, 64);
            //decoded_bitplanes_z = NegaDecode(encoded_bitplanes_z, nelems, level_exp, 64);
            double end_decode = MPI_Wtime ();
            printf("Encoding Time = %f, Decoding Time = %f\n", end_encode - start_encode, end_decode - end_encode);
            //for (i = 0; i < 50; i ++){
            //    printf("decoded_bitplanes[%d] = %f\n", i, decoded_bitplanes[i]);
            //}
            
            for (i = 0; i < 100; i ++){
                if (fabs(decoded_bitplanes[i] - delta[i]) > 0.0001)
                    printf("decoded_bitplanes[%d] = %f, delta[%d] = %f\n", i, decoded_bitplanes[i], i, delta[i]);
            }
            int bitplanes_len_nrmse[5] = {0, 7, 10, 14, 17};
            ***/
            
            double * delta_bitplane_block, *delta_r_bitplane_block, *delta_z_bitplane_block;
            int delta_bitplane_n;
            double start_bitplane = MPI_Wtime ();
            ...
            double middle_bitplane = MPI_Wtime ();
            ...
            double end_bitplane = MPI_Wtime ();
            printf("Time for original to bitplane: %f s\n", middle_bitplane - start_bitplane);
            ...
            int * bitplanes = (int *)malloc(target_nrmse_len *3* 4);
            for (int i = 0; i < target_nrmse_len; i++)
                bitplanes[i] = delta_bitplane_len[i];
            //for (int i = target_nrmse_len; i < 2 * target_nrmse_len; i++)
            //    bitplanes[i] = delta_r_bitplane_len[i];
            //for (int i = 2 * target_nrmse_len; i < 3 * target_nrmse_len; i++)
            //    bitplanes[i] = delta_z_bitplane_len[i];     
            
            
            //int delta_bitplane_n = (nelems/64 + 64) * num_bitplanes;
			if (!compress_delta)
			{ 	
                double compute_time = MPI_Wtime (); 
                //printf("Min of dalta_sorted = %f\n", get_min(delta_sorted, nelems));
                //for (int i = 0; i < nelems; i++)
                //    printf("delta_sorted[%d] = %f\n", i, delta_sorted[i]);
                printf("Start write\n");		
                
			    dptr = fopen("/hdd/delta_bitplane_xgc.bin","wb");
			    int delta_size=0, r_delta_size=0, z_delta_size=0, trunks_len_size;
			    delta_size = fwrite(delta_bitplane, sizeof(double), delta_bitplane_n, dptr);
			    ...
			    r_delta_size = fwrite(encoded_bitplanes_r, sizeof(double), delta_bitplane_n, dptr_r);
			    ...
			    z_delta_size = fwrite(encoded_bitplanes_z, sizeof(double), delta_bitplane_n, dptr_z);
                ...
                **/
                ...
                dptr = fopen("/ssd/bitplane_len_nrmse_xgc.bin","wb");
                z_delta_size = fwrite(delta_bitplane_len, sizeof(int), target_nrmse_len, dptr);
                if (z_delta_size != target_nrmse_len)
                    printf("Error writing delta_index, z_delta_size=%d, nelems=%d\n", z_delta_size, target_nrmse_len);    
                fflush(dptr);
                fclose(dptr);
