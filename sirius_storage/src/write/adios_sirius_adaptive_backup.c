...
//#ifdef CEPH
            ...
            */
		    ...
		    ptr = fopen("/ssd/reduced_data_xgc.bin","wb");
            ...
            //printf("data[0] = %f, data[1] = %f\n", data_original[0], data_original[1]);
            ...
            */
            ...
            for (int k = 0; k < nelems; k++){
                ...
                if (k<100)
                    printf("delta_index_sorted[%d] = %d\n",k,delta_index_sorted[k]);
            ...
            //PrintDouble(delta_sorted[0]);
            //double a[5] = {1.1, 1.2, 1.3, 1.4, 1.5};
            //BitplaneTransfer(a);
            //printf("a[3] = %f\n", a[3]);
            //printf("delta[1] = %f\n", delta[1]);
            //PrintDouble(delta[1], 0);
            double start_bitplane = MPI_Wtime ();
            int delta_extend_n = (nelems/64+1)*64;
            double * delta_extend = (double *)malloc (8 * delta_extend_n);
            memset(delta_extend, 0, sizeof(double) * delta_extend_n);
            double * bitblock = (double *)malloc (8 * 64);
            double * bitblock_new;
            for (int i = 0; i < nelems; i += 64){
                if ((i + 64) >= nelems){
                    memset(bitblock, 0, sizeof(double) * 64);
                    memcpy(bitblock, delta + i, 8 * (nelems - i));
                }
                else
                    memcpy(bitblock, delta + i, 8*64);
                //for (int j = 0; j < 64; j++)
                //    PrintDouble(bitblock[j], j);
            ...
            int *bitplane_len = get_bitplane_len_nrmse(delta_extend, delta_extend_n, delta, target_nrmse, target_nrmse_len, nrmse, nelems, get_max((double *)data, nelems), get_min((double *)data, nelems));
        ...
        //printf("delta_with_index[%d] = %f, original_nrmse_sq = %f, target_nrmse[%d]*target_nrmse[%d]=%f\n",i,delta_with_index[i].value,original_nrmse_sq,target_index,target_index,target_nrmse[target_index]*target_nrmse[target_index]);
            //double * bitplane = (double *)malloc (8 * nelems);
            //memset(bitplane, 0, sizeof(double) * nelems);
            //for (int i = 0; i < nelems; i+=64){
            //    block = (double *)malloc (8 * 64);
            //    memcpy(delta, block, 8*64);
            //    get_bitplane(delta, 64, );
            //get_bitplane_len_nrmse(bitplane, target_nrmse, target_nrmse_len, nrmse, nelems, get_max((double *)data, nelems), get_min((double *)data, nelems)); 
			if (!compress_delta)
			{ 	
                double start_write_time = MPI_Wtime (); 
			    ...
			    int delta_size=0, r_delta_size=0, z_delta_size=0, trunks_len_size;
			    delta_size = fwrite(delta_extend, sizeof(double), nelems, dptr);
			    if (delta_size != nelems)
				printf("Error writing delta, delta_size=%d, nelems=%d\n",delta_size, nelems);                      
			    fflush(dptr);
                fclose(dptr);
                printf("1\n");
                dptr_r = fopen("/hdd/delta_r_xgc.bin","wb");
			    r_delta_size = fwrite(delta_r, sizeof(double), nelems, dptr_r);
			    if (r_delta_size != nelems)
				    printf("Error writing r_delta, r_delta_size=%d, nelems=%d\n", r_delta_size, nelems);
			    fflush(dptr_r);
                fclose(dptr_r);
                printf("2\n");
                dptr_z = fopen("/hdd/delta_z_xgc.bin","wb");
			    z_delta_size = fwrite(delta_z, sizeof(double), nelems, dptr_z);
			    if (z_delta_size != nelems)
				    printf("Error writing z_delta, z_delta_size=%d, nelems=%d\n", z_delta_size, nelems);			    
			    fflush(dptr_z);
                ...
                dptr_z = fopen("/ssd/delta_bitplane_n_nrmse_xgc.bin","wb");
                z_delta_size = fwrite(bitplane_len, sizeof(int), target_nrmse_len, dptr_z);
                if (z_delta_size != nelems)
