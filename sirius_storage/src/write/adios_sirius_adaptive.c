...
double get_psnr_r (const double * orig, double * decoded, int nvertices, double org_max){
    double sum=0.0;
    double rmse = 0.0;
    double MSE = 0.0, temp_psnr = 0.0;
    for (int i = 0; i < nvertices; i++)
        sum += (orig[i] - decoded[i]) * (orig[i] - decoded[i]);

    MSE = sum / nvertices;
    if (MSE==0) printf ("MSE=0\n");
    temp_psnr = 10*log10(org_max * org_max/ MSE);
    return temp_psnr;
}
...
void get_psnr (double * delta, int nvertices, double org_max,  double *psnr)
{
    ...
    printf("MSE = %f, org_max = %f, psnr_o = %f, org_max * org_max/ MSE = %f\n", MSE, org_max, psnr_o, org_max * org_max/ MSE);
    *psnr=psnr_o;
    free(delta_o);
}
...
void decompress_new (double * delta_field, double * delta_r, double * delta_z, double * field_reduced, 
                double * r_reduced, double * z_reduced, int nvertices, int dec_ratio, 
                double ** rorg, double ** zorg, double ** fieldorg
              )
{
    double start_time = MPI_Wtime ();
    double * r, * z,* field;
    ...
    for (int i = 0; i < nvertices;){
        if ((i+dec_ratio) < nvertices)
            for (int j = 1; j < dec_ratio; j++){
                r[i+j] = (r[i]+r[i+dec_ratio])*j/dec_ratio + delta_r[i+j];
                z[i+j] = (z[i]+z[i+dec_ratio])*j/dec_ratio + delta_z[i+j];
                field[i+j] = (field[i]+field[i+dec_ratio])*j/dec_ratio + delta_field[i+j];  
            }
        else
            for (int j = 1; j < (nvertices-i); j++){
                r[i+j] = 2*r[i]*j/dec_ratio + delta_r[i+j];
                z[i+j] = 2*z[i]*j/dec_ratio + delta_z[i+j];
                field[i+j] = 2*field[i]*j/dec_ratio + delta_field[i+j];
            }
...
double get_NRMSE_r (const double * orig, double * decoded, int nvertices, double org_max, double org_min){
    double sum=0.0;
    double rmse = 0.0;
    for (int i = 0; i < nvertices; i++)
        sum += (orig[i] - decoded[i]) * (orig[i] - decoded[i]);
...
void Encode_block(double * bitblock, int blocksize, int bitplane_size, int num_bitplanes, int blockid, unsigned long long * delta_bitplane)
{
    for(int k=num_bitplanes - 1; k>=0; k--){
        unsigned long long bitplane_value = 0;
        int bitplane_index = num_bitplanes - 1 - k;
        for (int i=0; i<blocksize; i++){
            bitplane_value += ((* (unsigned long long *) &bitblock[i] >> k) & 1u) << i;
        }
        //if (blockid == 0) printf("bitplane_value = %lu\n", bitplane_value);
        delta_bitplane[bitplane_index * bitplane_size + blockid] = bitplane_value;
    }
}

void DoubleToBitplane(double * src, int n, unsigned long long ** delta_bitplanes, int * delta_transformed_n, int num_bitplanes){
    int block_size = 64;
    int bitplane_size = n/block_size+1;
    * delta_transformed_n = bitplane_size * num_bitplanes;
    unsigned long long * delta_new = (unsigned long long *)malloc (*delta_transformed_n * 8);
    memset(delta_new, 0, *delta_transformed_n * sizeof(double));
    double * bitblock = (double *)malloc (8 * block_size);
    double * bitblock_new;
    for (int i = 0; i < n; i += 64){
        if ((i + 64) >= n){
            int res_size = n - i;
            memset(bitblock, 0, sizeof(double) * 64);
            memcpy(bitblock, src + i, 8 * res_size);
        }
        else
            memcpy(bitblock, src + i, 8*64);
        Encode_block(bitblock, block_size, bitplane_size, num_bitplanes, i/block_size, delta_new);

    }
    * delta_bitplanes = delta_new;
}

void Decode_block(unsigned long long* encoded, unsigned long long * bitblock, int block_size, int bitplane_size, int num_bitplanes, int blockid, unsigned long long * bitblock_new)
{
    for(int k=num_bitplanes - 1; k>=0; k--){
        int bitplane_index = num_bitplanes - 1 - k;
        unsigned long long bitplane_value = encoded[bitplane_index * bitplane_size + blockid];
        //if (blockid == 0) printf("bitplane_value[bitplane_index] = %llu\n", bitplane_value);
        for (int i=0; i<block_size; i++){
            bitblock_new[i] = bitblock_new[i] | ((bitplane_value >> i) & 1u) << (63 - bitplane_index);
        }
    }
}
void BitplaneToDouble(unsigned long long * src, int src_n, int n, int num_bitplanes, double ** decoded){
    int block_size = 64;
    int bitplane_size = src_n / 64;
    //printf("src_n = %d, bitplane_size = %d\n", src_n, bitplane_size);
    unsigned long long * bitblock = (unsigned long long *)malloc (8 * block_size);
    unsigned long long * bitblock_new = (unsigned long long *)malloc (8 * block_size);
    //memset(bitblock_new, 0, sizeof(double) * 64);
    double * delta_new = (double *)malloc (8 * src_n);
    assert(bitblock && bitblock_new && delta_new);
    
    for (int i = 0; i < src_n; i+=block_size){
        memcpy(bitblock, src + i, 8*block_size);
        memset(bitblock_new, 0, sizeof(double) * block_size);
        Decode_block(src, bitblock, block_size, bitplane_size, num_bitplanes, i/block_size, bitblock_new);
        memcpy(delta_new + i, bitblock_new, 8*block_size);
    }
    //printf("Finish\n");
    * decoded = delta_new; 
}
...
/*
int* get_bitplane_len_nrmse(double * delta_extened, int delta_extend_n, double * delta_original, double *target_nrmse, int target_nrmse_n, double original_nrmse, int n, double max, double min)
{
    double original_nrmse_sq = original_nrmse * original_nrmse;
    double * delta_new  = (double *)malloc(n*8);
    int target_index=0;
    int* trunk_length;
    int prev_len = 0;
    int n_bitplane = 0;
    ...
    for (int i = 0; i <53; i++){
        BitplaneToDouble(delta_extened, delta_extend_n, n, i, &delta_new);
        sum = 0.0;
        //printf("delta_original[2] - delta_new[2] = %f\n", delta_original[i] - )
        for (int j = 0; j < n; j++)
            sum += (delta_original[j] - delta_new[j])* (delta_original[j] - delta_new[j]);
        temp_nrmse = sqrt(sum/n)/(max-min);
        printf("%d bitplanes nrmse = %f\n", i, temp_nrmse);
        if (temp_nrmse <= target_nrmse[target_index]){
            trunk_length[target_index] = i;
            printf("Number of bitplanes needed for %f: %d\n", target_nrmse[target_index], i);
            target_index+=1;
        }
        if (target_index == target_nrmse_n){
            for(int m = 0; m < target_nrmse_n; m++)
...
void get_bitplane_representation(double *FPstandard, int FP_n, double ** transformed_FPstandard){
    double * new_FPstandard = (double *)malloc(FP_n * sizeof(double));
    int n_in_onebp = FP_n/64;
    for (int i = 0; i < FP_n; i++){
        *(new_FPstandard + (i % 64) * n_in_onebp + i / 64) = FPstandard[i];    
    }
    * transformed_FPstandard = new_FPstandard;
}
*/
...
void PrintDoubleBinary(unsigned long long i)
{
    printf("Binary_value:");
    for (int j = 63; j >= 0; j--)
    {
...
/*
void get_bitplane(double * src, int n, int * bitplane)
{   
    for (int i = 0; i < n; i++)
    {
        unsigned long long p = * (unsigned long long *) &src[i];
        for (int j = 63; j >= 0; j--)
        {   
            int tmp = (p>>j)&1;
            bitplane[63 - j]<<1;
            bitplane[63 - j] = bitplane[63 - j] || tmp;
        }
    }
}
*/
...
void encode_block(uint64_t const * data, size_t n, uint8_t num_bitplanes, uint64_t* streams, int bitplane_size, int blockid){
    for(int k=num_bitplanes - 1; k>=0; k--){
        uint64_t bitplane_value = 0;
        uint64_t bitplane_index = num_bitplanes - 1 - k;
        for (int i=0; i<n; i++){
            bitplane_value = bitplane_value | ((uint64_t)((data[i] >> k) & 1u) << i);
        }
        streams[bitplane_index * bitplane_size + blockid] = bitplane_value;
    }
}

uint64_t* NegaEncode(double const * data, int n, int exp, int num_bitplanes)
{   
    assert(num_bitplanes > 0);
    exp += 2;
    uint32_t block_size = 64;
    int bitplane_size = n / 64 + 1;
    //printf("bitplane_size = %d\n", bitplane_size);
    int offset = 0; 

    uint64_t * streams = (uint64_t *)malloc(bitplane_size * num_bitplanes * sizeof(uint64_t));
    uint64_t * int_data_buffer = (uint64_t *)calloc(block_size, sizeof(uint64_t));
    double const * data_pos = data;
    for(int i=0; i<n - block_size; i+=block_size){
        for(int j=0; j<block_size; j++){
            double cur_data = *(data_pos + offset);
            offset += 1;
            double shifted_data = ldexp(cur_data, num_bitplanes - exp);
            int_data_buffer[j] = binary2negabinary((int64_t) shifted_data);
        }
        encode_block(int_data_buffer, block_size, num_bitplanes, streams, bitplane_size, i/block_size);
    }
    {   
        ...
        encode_block(int_data_buffer, rest_size, num_bitplanes, streams, bitplane_size, bitplane_size -1);
    }
    return streams;
}
...
void decode_block(uint64_t const * streams, size_t n, uint8_t num_bitplanes, uint64_t * data, int bitplane_size, int blockid) {
    for(int k=num_bitplanes - 1; k>=0; k--){
        uint64_t bitplane_index = num_bitplanes - 1 - k;
        uint64_t bitplane_value = streams[bitplane_index * bitplane_size + blockid]; 
        for (int i=0; i<n; i++){
            data[i] = data[i] | (((bitplane_value >> i) & 1u) << k);
        }
    }
}
double * NegaDecode(uint64_t * streams, int n, int exp, int num_bitplanes) {
    int block_size = 64;
    int bitplane_size = n / block_size + 1;
...
int* get_n_bitplanes_rmse(double * original, uint64_t * encoded_bitplanes, int encode_num_bitplanes, int n, int level_exp, double *target_nrmse, int target_nrmse_n, double original_nrmse, double max, double min){
    double * decoded;
    int target_index = 0;
    int * trunk_length = (int *)malloc(target_nrmse_n*4);
    double tmp_rmse;
    for (int i = 0; i < target_nrmse_n; i++){
        if(target_nrmse[i] < original_nrmse){
            target_index = i;
    ...
    for (int i = 1; i < encode_num_bitplanes + 1; i++){
        decoded = NegaDecode(encoded_bitplanes, n, level_exp, i);
        tmp_rmse = get_NRMSE_r(original, decoded, n, max, min);
        //printf("Read %d bitplanes, NRMSE = %f\n", i, tmp_rmse);
        if (tmp_rmse < target_nrmse[target_index]){
            printf("Require %d bitplanes to reach NRMSE = %f, actual NRMSE = %f\n", i, target_nrmse[target_index], tmp_rmse);
            trunk_length[target_index] = i;
            target_index += 1;
            if (target_index >= target_nrmse_n) break;
        }
    }
    return trunk_length;
}

int* get_n_bitplanes_psnr(double * original, uint64_t * encoded_bitplanes, int encode_num_bitplanes, int n, int level_exp, double *target_psnr, int target_psnr_n, double original_psnr, double max, double min){
    double * decoded;
    int target_index = 0;
    int * trunk_length = (int *)malloc(target_psnr_n*4);
    double tmp_psnr;
    for (int i = 0; i < target_psnr_n; i++){
        if(target_psnr[i] < original_psnr){
            target_index = i;
    ...
    for (int i = 1; i < encode_num_bitplanes + 1; i++){
        decoded = NegaDecode(encoded_bitplanes, n, level_exp, i);
        tmp_psnr = get_psnr_r(original, decoded, n, max);
        //printf("Read %d bitplanes, PSNR = %f\n", i, tmp_psnr);
        if (tmp_psnr > target_psnr[target_index]){
            printf("Require %d bitplanes to reach PSNR = %f, actual PSNR = %f\n", i, target_psnr[target_index], tmp_psnr);
            trunk_length[target_index] = i;
            target_index += 1;
            if (target_index >= target_psnr_n) break;
        }
    }
    return trunk_length;
}
...

{
    ...
    double * data_decompressed, * r_decompressed, * z_decompressed;
...
//                        printf ("level = %d, ntaggedCells = %d\n", l, ntaggedCells);
                        ...
                        */
                        ...
                        if (save_delta)
                        {
                            ...
                            decompress_new (delta, delta_r, delta_z, data_reduced,
                                        r_reduced, z_reduced, nelems, dec_ratio,
                                        &r_decompressed, &z_decompressed, &data_decompressed
                                        );
...
//#ifdef CEPH
            /*
            ptr = fopen("/ssd/reduced_data_xgc_test.bin","wb"); 
            int test_size=0 ;
            ...
            */
		    double start_time = MPI_Wtime ();
			char reduced_name[64];
            sprintf (reduced_name, "/ssd/reduced_data_xgc_%d.bin", dec_ratio);    
		    ptr = fopen(reduced_name,"wb");
		    ...
		    double * data_original = (double*)data;
            double io_time1 = MPI_Wtime ();
            
		    fptr = fopen("/ssd/full_data_xgc.bin","wb");
		    int data_f_size=0, r_f_size=0, z_f_size=0;
		    data_f_size = fwrite((double*)data, sizeof(double), nelems, fptr);
		    if (data_f_size != nelems)
			    printf("Error writing data, data_f_size=%d, nelems=%lu\n",data_f_size, nelems);
            
		    fflush(fptr);
		    r_f_size = fwrite(rr, sizeof(double), nelems, fptr);
		    if (r_f_size != nelems)
			printf("Error writing R, r_f_size=%d, nelems=%lu\n", r_f_size, nelems);

		    fflush(fptr);
		    z_f_size = fwrite(zz, sizeof(double), nelems, fptr);
		    if (z_f_size != nelems)
			printf("Error writing Z, z_f_size=%d, nelems=%lu\n", z_f_size, nelems);
		    ...
		    {	
                ...
                dptr = fopen("/hdd/delta_xgc_o.bin","wb");
                ...
                printf("Error writing delta, delta_size=%d, nelems=%lu\n",delta_size, nelems);           
                fflush(dptr);
                fclose(dptr);
                
                dptr_r = fopen("/hdd/delta_r_xgc_o.bin","wb");
                r_delta_size = fwrite(delta_r, sizeof(double), nelems, dptr_r);
                if (r_delta_size != nelems)
                    printf("Error writing r_delta, r_delta_size=%d, nelems=%lu\n", r_delta_size, nelems);
                fflush(dptr_r);
                fclose(dptr_r);
                printf("2\n");
                dptr_z = fopen("/hdd/delta_z_xgc_o.bin","wb");
                z_delta_size = fwrite(delta_z, sizeof(double), nelems, dptr_z);
                if (z_delta_size != nelems)
                printf("Error writing z_delta, z_delta_size=%d, nelems=%lu\n", z_delta_size, nelems);    
                fflush(dptr_z);
                fclose(dptr_z);
            ...
            double nrmse_r = get_NRMSE(delta_r, nelems, get_max((double *)rr, nelems),get_min((double *)rr, nelems));
            double nrmse_z = get_NRMSE(delta_z, nelems, get_max((double *)zz, nelems),get_min((double *)zz, nelems));
            get_psnr (delta, nelems, get_max((double *)data, nelems),  &psnr);
            get_psnr (delta_r, nelems, get_max((double *)rr, nelems),  &psnr_r);
            get_psnr (delta_z, nelems, get_max((double *)zz, nelems),  &psnr_z);
            printf("PSNR = %f, PSNR_r = %f, PSNR_z = %f\n", psnr, psnr_r, psnr_z);
            double target_nrmse[4] = {0.01, 0.001, 0.0001, 0.00001};
            //printf("target_nrmse_length = %d\n", sizeof(target_nrmse)/sizeof(target_nrmse[0]));
            double target_psnr[6] = {30 ,40, 50, 60, 70, 80};
			...
			printf("target_nrmse_len = %d, target_psnr_len = %d\n", target_nrmse_len, target_psnr_len);
			printf("NRMSE = %f, NRMSE_r = %f, NRMSE_z = %f\n", nrmse, nrmse_r, nrmse_z);
			if (target_nrmse[0] > 1)
            ...
            /*
            qsort(gradient_with_index, nvertices_new, sizeof(gradient_with_index[0]), cmp1);
            ...
            dptr = fopen("/hdd/delta_sorted_gradient_xgc.bin", "wb");
            ...
            dptr = fopen("/hdd/delta_r_sorted_gradient_xgc.bin", "wb");
            ...
            dptr = fopen("/hdd/delta_z_sorted_gradient_xgc.bin", "wb");
            ...
            dptr = fopen("/ssd/delta_index_sorted_gradient_xgc.bin", "wb");
            ...
            */
            ...
            double start_bitplane = MPI_Wtime ();
            double delta_absmax = 0.0, delta_r_absmax = 0.0, delta_z_absmax = 0.0;
            for (int i = 0; i < nelems; i++){
                if (fabs(delta[i]) > delta_absmax)    delta_absmax = fabs(delta[i]);
                if (fabs(delta_r[i]) > delta_r_absmax)    delta_r_absmax = fabs(delta_r[i]);
                if (fabs(delta_z[i]) > delta_z_absmax)    delta_z_absmax = fabs(delta_z[i]);
            }
            
            int delta_level_exp = 0, delta_r_level_exp = 0, delta_z_level_exp = 0;
            frexp(delta_absmax, &delta_level_exp);   
            frexp(delta_r_absmax, &delta_r_level_exp);
            frexp(delta_z_absmax, &delta_z_level_exp);
            printf("delta_level_exp = %d\n", delta_level_exp);
            printf("delta_r_level_exp = %d\n", delta_r_level_exp);
            printf("delta_z_level_exp = %d\n", delta_z_level_exp);
                
            int encode_num_bitplanes = 64;     
            int bitplane_size = nelems/64 + 1;
            int delta_bitplane_n = bitplane_size * encode_num_bitplanes;
            double start_encode = MPI_Wtime ();   
            uint64_t * encoded_bitplanes, *encoded_bitplanes_r, *encoded_bitplanes_z; 
            encoded_bitplanes = NegaEncode(delta, nelems, delta_level_exp, encode_num_bitplanes);
            encoded_bitplanes_r = NegaEncode(delta_r, nelems, delta_r_level_exp, encode_num_bitplanes);
            encoded_bitplanes_z = NegaEncode(delta_z, nelems, delta_z_level_exp, encode_num_bitplanes);
            double end_encode = MPI_Wtime ();  

            for (int i = 0; i < delta_bitplane_n; i += bitplane_size){
                printf("encoded_bitplanes[%d] = %lu\n", (int)i/bitplane_size, encoded_bitplanes[i]);
            }

            int decode_num_bitplanes = 20;
            double * decoded_bitplanes, *decoded_bitplanes_r, *decoded_bitplanes_z;

            //for (int i = 0; i < target_nrmse_len; i++){
            //    printf("delta_n_bitplane_nrmse[%d] = %d\n", i, delta_n_bitplane_nrmse[i]);
            //}
            decoded_bitplanes = NegaDecode(encoded_bitplanes, nelems, delta_level_exp, decode_num_bitplanes);
            double end_decode_delta = MPI_Wtime ();
            printf("Decode delta time = %f\n", end_decode_delta - end_encode);
            for (int i = 0; i < 100; i++)
                printf("delta[%d] = %f\n", i, decoded_bitplanes[i]);
            decoded_bitplanes_r = NegaDecode(encoded_bitplanes_r, nelems, delta_r_level_exp, decode_num_bitplanes);
            decoded_bitplanes_z = NegaDecode(encoded_bitplanes_z, nelems, delta_z_level_exp, decode_num_bitplanes);
            double end_decode = MPI_Wtime ();
            int * delta_n_bitplane_nrmse = get_n_bitplanes_rmse(delta, encoded_bitplanes, encode_num_bitplanes, nelems, delta_level_exp, target_nrmse, target_nrmse_len, nrmse, get_max((double *)data, nelems), get_min((double *)data, nelems));
            int * delta_r_n_bitplane_nrmse = get_n_bitplanes_rmse(delta_r, encoded_bitplanes_r, encode_num_bitplanes, nelems, delta_r_level_exp, target_nrmse, target_nrmse_len, nrmse_r, get_max((double *)rr, nelems), get_min((double *)rr, nelems));
            int * delta_z_n_bitplane_nrmse = get_n_bitplanes_rmse(delta_z, encoded_bitplanes_z, encode_num_bitplanes, nelems, delta_z_level_exp, target_nrmse, target_nrmse_len, nrmse_z, get_max((double *)zz, nelems), get_min((double *)zz, nelems));

            int * delta_n_bitplane_psnr = get_n_bitplanes_psnr(delta, encoded_bitplanes, encode_num_bitplanes, nelems, delta_level_exp, target_psnr, target_psnr_len, psnr, get_max((double *)data, nelems), get_min((double *)data, nelems));
            int * delta_r_n_bitplane_psnr = get_n_bitplanes_psnr(delta_r, encoded_bitplanes_r, encode_num_bitplanes, nelems, delta_r_level_exp, target_psnr, target_psnr_len, psnr_r, get_max((double *)rr, nelems), get_min((double *)rr, nelems));
            int * delta_z_n_bitplane_psnr = get_n_bitplanes_psnr(delta_z, encoded_bitplanes_z, encode_num_bitplanes, nelems, delta_z_level_exp, target_psnr, target_psnr_len, psnr_z, get_max((double *)zz, nelems), get_min((double *)zz, nelems));

            double end_cal_rmse = MPI_Wtime ();
            printf("Encoding Time = %f, Decoding Time = %f, Calculate required number of bitplanes to reach target NRMSE Time = %f\n", end_encode - start_encode, end_decode - end_encode, end_cal_rmse - end_decode);
            
            for (int i = 0; i < target_nrmse_len; i++){
                printf("delta_n_bitplane_nrmse[%d] = %d, delta_r_n_bitplane_nrmse[%d] = %d, delta_z_n_bitplane_nrmse[%d] = %d\n", i, delta_n_bitplane_nrmse[i], i, delta_r_n_bitplane_nrmse[i], i, delta_z_n_bitplane_nrmse[i]);
            }
            // 754 unsigned long long * delta_bitplanes;
            // 754 int delta_bitplane_n;
            // 754 int encode_num_bitplanes = 64;
            // 754 int decode_num_bitplanes = 16;
            // 754 double * delta_decoded = 0;
            // 754 double start_bitplane = MPI_Wtime ();
            // 754 DoubleToBitplane(delta, nelems, &delta_bitplanes, &delta_bitplane_n, encode_num_bitplanes);
            //DoubleToBitplaneBlock(delta, nelems, &delta_bitplane_block, &delta_bitplane_n);
            //DoubleToBitplaneBlock(delta_r, nelems, &delta_r_bitplane_block, &delta_bitplane_n);
            //DoubleToBitplaneBlock(delta_z, nelems, &delta_z_bitplane_block, &delta_bitplane_n);
            //printf("nelems = %d\n", nelems);
            //printf("delta_bitplane_n = %d\n", delta_bitplane_n);
            
            // 754 double middle_bitplane = MPI_Wtime ();
            // 754 BitplaneToDouble(delta_bitplanes, delta_bitplane_n, nelems, decode_num_bitplanes, &delta_decoded);
            //int *delta_bitplane_len = get_bitplane_len_nrmse(delta_bitplane_block, delta_bitplane_n, delta, target_nrmse, target_nrmse_len, nrmse, nelems, get_max((double *)data, nelems), get_min((double *)data, nelems));
            //int *delta_r_bitplane_len = get_bitplane_len_nrmse(delta_r_bitplane_block, delta_bitplane_n, delta_r, target_nrmse, target_nrmse_len, nrmse_r, nelems, get_max((double *)rr, nelems), get_min((double *)rr, nelems));
            //int *delta_z_bitplane_len = get_bitplane_len_nrmse(delta_z_bitplane_block, delta_bitplane_n, delta_z, target_nrmse, target_nrmse_len, nrmse_z, nelems, get_max((double *)zz, nelems), get_min((double *)zz, nelems));
            //double *delta_bitplane, *delta_r_bitplane, *delta_z_bitplane;
            //get_bitplane_representation(delta_bitplane_block, delta_bitplane_n, &delta_bitplane);
            //get_bitplane_representation(delta_r_bitplane_block, delta_bitplane_n, &delta_r_bitplane);
            //get_bitplane_representation(delta_z_bitplane_block, delta_bitplane_n, &delta_z_bitplane); 
            // 754 double end_bitplane = MPI_Wtime ();
            // 754 printf("Time for encode: %f s\n", middle_bitplane - start_bitplane);
            // 754 printf("Time for decode : %f s\n", end_bitplane - middle_bitplane);
            //printf("Time for calculating required number of bitplanes for target error bound: %f s\n", end_bitplane - middle_bitplane);
            //for (int i = 0; i < target_nrmse_len; i++){
                //printf("bitplane_len[%d] = %d\n", i, delta_bitplane_len[i]);
                //printf("bitplane_r_len[%d] = %d\n", i, delta_r_bitplane_len[i]); 
                //printf("bitplane_z_len[%d] = %d\n", i, delta_z_bitplane_len[i]);
            //}
            int meta_len_nrmse = target_nrmse_len * 3 + 3;
            int * n_bitplanes_nrmse = (int *)malloc(meta_len_nrmse * sizeof(int));
            for (int i = 0; i < target_nrmse_len; i++){
                n_bitplanes_nrmse[i] = delta_n_bitplane_nrmse[i];
                n_bitplanes_nrmse[target_nrmse_len + i] = delta_r_n_bitplane_nrmse[i];
                n_bitplanes_nrmse[target_nrmse_len * 2 + i] = delta_z_n_bitplane_nrmse[i];
            }
            n_bitplanes_nrmse[target_nrmse_len * 3] = delta_level_exp;
            n_bitplanes_nrmse[target_nrmse_len * 3 + 1] = delta_r_level_exp;
            n_bitplanes_nrmse[target_nrmse_len * 3 + 2] = delta_z_level_exp;
            for(int i = 0; i < meta_len_nrmse; i ++)
                printf("meta_nrmse[%d] = %d\n", i, n_bitplanes_nrmse[i]);

            int meta_len_psnr = target_psnr_len * 3 + 3;
            int * n_bitplanes_psnr = (int *)malloc(meta_len_psnr * sizeof(int));
            for (int i = 0; i < target_psnr_len; i++){
                n_bitplanes_psnr[i] = delta_n_bitplane_psnr[i];
                n_bitplanes_psnr[target_psnr_len + i] = delta_r_n_bitplane_psnr[i];
                n_bitplanes_psnr[target_psnr_len * 2 + i] = delta_z_n_bitplane_psnr[i];
            }
            n_bitplanes_psnr[target_psnr_len * 3] = delta_level_exp;
            n_bitplanes_psnr[target_psnr_len * 3 + 1] = delta_r_level_exp;
            n_bitplanes_psnr[target_psnr_len * 3 + 2] = delta_z_level_exp;
            for(int i = 0; i < meta_len_psnr; i ++)
                printf("meta_psnr[%d] = %d\n", i, n_bitplanes_psnr[i]);
			...
			{ 	
                ...
                dptr = fopen("/hdd/delta_bitplane_20_xgc.bin","wb");
                delta_size = fwrite(encoded_bitplanes, sizeof(unsigned long long), bitplane_size * 20, dptr);
                if (delta_size != bitplane_size * 20)
                    printf("Error writing delta, delta_size=%d, delta_bitplane_n=%d\n",delta_size, bitplane_size * 20);
                fflush(dptr);
                fclose(dptr);

			    dptr = fopen("/hdd/delta_bitplane_xgc.bin","wb");
			    delta_size = fwrite(encoded_bitplanes, sizeof(unsigned long long), delta_bitplane_n, dptr);
			    if (delta_size != delta_bitplane_n)
				    printf("Error writing delta, delta_size=%d, delta_bitplane_n=%d\n",delta_size, delta_bitplane_n);                      
			    fflush(dptr);
                fclose(dptr);
                 
                dptr_r = fopen("/hdd/delta_r_bitplane_xgc.bin","wb");
			    r_delta_size = fwrite(encoded_bitplanes_r, sizeof(unsigned long long), delta_bitplane_n, dptr_r);
			    if (r_delta_size != delta_bitplane_n)
				    printf("Error writing r_delta, r_delta_size=%d, delta_bitplane_n=%d\n", r_delta_size, delta_bitplane_n);
			    fflush(dptr_r);
                fclose(dptr_r);

                dptr_z = fopen("/hdd/delta_z_bitplane_xgc.bin","wb");
			    z_delta_size = fwrite(encoded_bitplanes_z, sizeof(unsigned long long), delta_bitplane_n, dptr_z);
			    if (z_delta_size != delta_bitplane_n)
				    printf("Error writing z_delta, z_delta_size=%d, delta_bitplane_n=%d\n", z_delta_size, delta_bitplane_n);			    
			    fflush(dptr_z);
                fclose(dptr_z);
                
                dptr = fopen("/ssd/metadata_nrmse_xgc.bin","wb");
                z_delta_size = fwrite(n_bitplanes_nrmse, sizeof(int), meta_len_nrmse, dptr);
                if (z_delta_size != meta_len_nrmse)
                    printf("Error writing delta_index, z_delta_size=%d, meta_len_nrmse=%d\n", z_delta_size, meta_len_nrmse);    
                fflush(dptr);
                fclose(dptr);

                dptr = fopen("/ssd/metadata_psnr_xgc.bin","wb");
                z_delta_size = fwrite(n_bitplanes_psnr, sizeof(int), meta_len_psnr, dptr);
                if (z_delta_size != meta_len_psnr)
                    printf("Error writing delta_index, z_delta_size=%d, meta_len_psnr=%d\n", z_delta_size, meta_len_psnr);
                fflush(dptr);
                fclose(dptr);

                dptr = fopen("/hdd/delta_sorted_xgc.bin","wb");
                ...
                dptr_r = fopen("/hdd/delta_r_sorted_xgc.bin","wb");
                ...
                dptr_z = fopen("/hdd/delta_z_sorted_xgc.bin","wb");
                ...
                dptr_z = fopen("/hdd/delta_index_sorted_xgc.bin","wb");
                z_delta_size = fwrite(delta_index_sorted, sizeof(int), nelems, dptr_z);
                if (z_delta_size != nelems)
                    printf("Error writing delta_index, z_delta_size=%d, nelems=%lu\n", z_delta_size, nelems);    
                fflush(dptr_z);
                fclose(dptr_z);     
                
                dptr_trunk_len_nrmse = fopen("/ssd/trunk_len_nrmse_xgc.bin","wb");
                ...
                dptr_trunk_len_psnr = fopen("/ssd/trunk_len_psnr_xgc.bin","wb");
			    ...
			    double end_time = MPI_Wtime ();
		 	    printf("Write io time = %f\n",end_time - compute_time + io_time1 - start_time); 
                printf("Compute time = %f\n",compute_time - io_time1);
