...
def fully_refine(full_weight_bool, ssd_read_time, full_read_time, full_all_delta_read_time, deci_ratio,timestep,tag,time_tail,weight_bool, fulldata_len, reduced_len,docker_path_id, error_metric, error_target, nrmse_target_list, psnr_target_list, priority, min_priority, max_priority, min_accuracy, max_accuracy, weight_tag):
    ...
    filename = ssd_path+"reduced_data_"+application_name+"_3.bin"
    f = open(filename, "rb")
...
def partial_refine(pool, low_accuracy_read_time, ssd_read_time, decode_time, full_read_time, deci_ratio,timestep,psnr,ctag,time_tail, peak_noise, no_noise, time_interval, fulldata_len,reduced_len,docker_path_id,weight_bool,error_control_bool,error_metric, error_target, nrmse_target_list, psnr_target_list, priority, min_priority, max_priority, min_accuracy, max_accuracy, weight_tag):
    ...
    if sig > -1:
        ...
        #for i in range(100):
        #    print("delta_L0_L1[%d] = %f"%(i, delta_L0_L1[i])
            
        #bit.NegaDecode(input_len(*delta_L0_L1_encoded), fulldata_len, delta_level_exp, n_bitplanes_decode)
...
def work_flow(full_weight_bool, deci_ratio, time_interval, psnr,peak_noise, no_noise, fulldata_len,reduced_len,docker_path_id, weight_bool,error_control_bool,error_metric, error_target, nrmse_target_list, psnr_target_list, priority, min_priority, max_priority, min_accuracy, max_accuracy, weight_tag, only_partial):
    ...
    for i in range(start_time, end_time ,time_interval):
        ...
        # partial read
            ...
            w_end = time.time()
            print("Phase I time = ", w_end - start)
            partial_refine(pool, low_accuracy_read_time, ssd_read_time, decode_time, all_partial_delta_read_time, deci_ratio, timestep, psnr, tag,t_tail, peak_noise, no_noise, time_interval, fulldata_len, reduced_len, docker_path_id,weight_bool,error_control_bool,error_metric, error_target, nrmse_target_list, psnr_target_list, priority, min_priority, max_priority, min_accuracy, max_accuracy, weight_tag)
        end = time.time()
...
docker_path_id = "aa708b28824e8eea8b68d4f6c1facefbaa2987b81cea9377380b14192e452ccd"
ssd_path = "/ssd/"
hdd_path = "/hdd/"
...
def main():
    ...
    while (tag):
        ...
        if sys.argv[3] == 'now':
            work_flow(full_weight_bool, deci_ratio,time_interval,"false", peak_noise, no_noise, fulldata_len,reduced_len, docker_path_id, weight_bool,error_control_bool,error_metric, error_target, nrmse_target_list, psnr_target_list, priority, min_priority, max_priority, min_accuracy, max_accuracy, weight_tag, only_partial)
            tag = 0
            break
